...
final class AIService {
    ...
    func chat(payload: [String: Any]) async throws -> [String: Any] {
        ...
        {"text":"concise answer","suggestions":[{"type":"draftEntry","start":0,"end":0,"description":"","projectId":"","taskId":"","billable":false},{"type":"draftActivityGroup","title":"","candidateIds":["draft-candidate-1"],"projectId":"","taskId":"","confidence":"high","evidence":""},{"type":"updateAssignment","entryId":"","projectId":"","taskId":""}]}
        ...
        Use project descriptions, project names, task names, apps, domains, titles, and activity timing as evidence when matching captured activity to projects.
        If the evidence for a project is weak, explain the uncertainty in text without returning a suggestion for that activity.
        ...
        Group generic entry suggestions by project and topic when draftMode is "activitySet". For those grouped suggestions, return draftActivityGroup items that reference only IDs from dayContext.draftCandidates in candidateIds and include a valid projectId from dayContext.projects. Do not return draftEntry items for draftMode "activitySet".
        Do not return draftActivityGroup items unless there is enough evidence to assign the group to a concrete project.
        """
    ...
    private func sanitizeIntent(_ intent: [String: Any]?) -> [String: Any] {
        ...
        let rawDraftMode = intent?["draftMode"] as? String ?? ""
        if ["activitySet", "singleRange"].contains(rawDraftMode) {
            sanitized["draftMode"] = rawDraftMode
        }
        return sanitized
    }
